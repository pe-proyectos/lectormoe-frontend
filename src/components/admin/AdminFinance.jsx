...
export function AdminFinance({ language }) {
        ...
        : transactions.filter((t) => {
            ...
            return (
              `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(
                2,
                "0"
              )}` === selectedMonth && t.status === "COMPLETED"
            );
          });
    ...
    return {
      options: {
        ...
        xaxis: {
          ...
          title: {
            text: {
              monthly: "Transacciones por mes",
              weekly: "Transacciones por semana",
              daily: "Transacciones por dia",
            }[graphInterval],
          },
        },
        yaxis: {
  ...
  const refreshTransactions = () => {
    ...
    callAPI(`/api/transactions`)
      ...
      .catch((error) =>
        toast.error(error?.message || _("error_loading_transactions"))
      )
      ...
      <Card className="mb-4">
        <CardHeader floated={false} shadow={false}>
          <div className="flex justify-between items-center">
            <Typography variant="h6">Ganancias en el tiempo</Typography>
                  ...
                  <>
                    <tr key={transaction.id}>
                      ...
                      <td className="p-4 border-b border-blue-gray-50">
                        <Typography
                          variant="small"
                          color={
                            transaction.type === "EARNING" ? "green" : "red"
                          }
                        >
                          {formatCurrency(
                            transaction.amount,
                    ...
                    {expandedRows[transaction.id] && (
                      <tr>
                        <td
                          colSpan={6}
                          className="p-2 border-b border-blue-gray-50 bg-blue-gray-50/50"
                        >
