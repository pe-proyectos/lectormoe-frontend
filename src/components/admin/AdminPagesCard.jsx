...
import { Card, Button, Switch, Typography } from "@material-tailwind/react";
...
export function AdminPagesCard({ language, manga, chapter, pages }) {
  ...
  const saveNewOrder = async () => {
    callAPI(
      `/api/manga-custom/${manga.slug}/chapter/${chapter.number}/pages/order`,
      {
        method: "POST",
        body: JSON.stringify(
          data.map((page, index) => ({
  ...
  const deletePage = async (id) => {
    callAPI(
      `/api/manga-custom/${manga.slug}/chapter/${chapter.number}/pages/${id}`,
      {
        method: "DELETE",
      }
    )
      ...
      <div className="dropzone-container flex flex-col items-end justify-end">
        ...
        <div>
          <Button
            ...
            variant="text"
            className="my-6 mx-1"
            onClick={() => setOpenUploadImagesDialog(true)}
          >
            {_("upload_images")}
          </Button>
          <Button
