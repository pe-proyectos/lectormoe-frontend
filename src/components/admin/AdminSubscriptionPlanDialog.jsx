...
export function AdminSubscriptionPlanDialog({
  language,
  open,
  setOpen,
  subscriptionPlan,
  ...
  const handleSubmit = async () => {
    ...
    callAPI(
      subscriptionPlan
        ? `/api/subscription-plan/${subscriptionPlan.id}`
    ...
    >
      <DialogHeader>
        <Typography variant="h4" color="blue-gray">
          {subscriptionPlan
            ? _("edit_subscription_plan")
            : _("create_subscription_plan")}
        </Typography>
      </DialogHeader>
      <DialogBody className="max-h-[65vh] overflow-y-auto flex flex-col gap-4">
        ...
        <div className="w-full">
          ...
          >
            <Option value="DAY" selected={planInterval === "DAY"}>
              {_("daily")}
            </Option>
            <Option value="WEEK" selected={planInterval === "WEEK"}>
              {_("weekly")}
            </Option>
            <Option value="MONTH" selected={planInterval === "MONTH"}>
              {_("monthly")}
            </Option>
            <Option value="YEAR" selected={planInterval === "YEAR"}>
              {_("yearly")}
            </Option>
          </Select>
        </div>
