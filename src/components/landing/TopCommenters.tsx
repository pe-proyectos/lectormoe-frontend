...
import { MessageSquare, Medal, TrendingUp } from 'lucide-react';
...
const TopCommenters: React.FC = () => {
  ...
  useEffect(() => {
    const fetchTopCommenters = async () => {
      try {
        ...
        const response = await fetch(`${API_URL}/api/landing/top-commenters`);
        ...
        console.error('Error fetching global top commenters:', error);
      } finally {
        setLoading(false);
      }
    };
  ...
  if (!loading && commenters.length === 0) return null;
    ...
    <section className="max-w-[1600px] mx-auto px-3 md:px-8 py-16">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8">
        <div className="space-y-1">
          ...
          <h2 className="text-3xl font-black text-white italic tracking-tighter uppercase leading-none">Top Comentaristas de la Semana</h2>
        </div>
      </div>
            ...
            return (
              ...
              >
                <div className="flex flex-col items-center gap-3">
                  ...
                  <div className="w-14 h-14 rounded-full bg-zinc-900 border border-zinc-800 overflow-hidden flex items-center justify-center text-zinc-400 font-black text-lg group-hover:border-purple-500/50 transition-colors">
                    {commenter.imageUrl ? (
                      <img
                  ...
                  <div className="text-center min-w-0 w-full">
                    <p className="text-white font-bold text-sm leading-none mb-1 truncate">{commenter.username}</p>
