...
const PostComposer: React.FC<Props> = ({ user, language = 'es', onCreated, replyTo, asOrgSlug, placeholder, compact, autoFocus }) => {
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (!replyTo && !compact) toast.success(en ? 'Posted ✓' : 'En la charca ✓')
  ...
  return (
    <div className="flex gap-3 px-4 py-3">
      ...
      <div className="flex-1 min-w-0">
        ...
        <textarea value={content} onChange={(e) => setContent(e.target.value)} rows={compact ? 1 : 2}
          autoFocus={autoFocus}
          onKeyDown={(e) => { if ((e.metaKey || e.ctrlKey) && e.key === 'Enter') submit() }}
          placeholder={placeholder || (replyTo ? (en ? 'Join the conversation' : 'Súmate a la conversación') : (en ? "What are you reading?" : '¿Qué cuentas hoy?'))}
        ...
        {spoiler && (
          <div className="mt-3 rounded-2xl border border-amber-500/20 bg-amber-500/[0.04] p-3 space-y-2">
            <p className="text-[11px] font-black uppercase tracking-wider text-amber-400/80">{en ? 'Spoiler shield (optional)' : 'Escudo antispoiler (opcional)'}</p>
            {spoilerWork ? (
        ...
        <div className="mt-2 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            {content.length > 4500 && <span className={`text-[11px] font-bold tabular-nums ${over ? 'text-rose-400' : content.length > 4800 ? 'text-amber-400' : 'text-white/40'}`}>{5000 - content.length}</span>}
          <button type="button" onClick={submit} disabled={busy || over || (!content.trim() && files.length === 0 && !hasPoll)}
            className="flex items-center gap-1.5 px-5 py-2 rounded-full bg-gradient-to-br from-teal-400 to-cyan-500 text-zinc-950 text-sm font-black hover:from-teal-300 hover:to-cyan-400 shadow-[0_1px_0_rgba(255,255,255,0.3)_inset,0_6px_18px_-6px_rgba(45,212,191,0.55)] disabled:opacity-40 disabled:shadow-none active:scale-[0.98] transition cursor-pointer">
