...
const MangaDetailPage: React.FC<MangaDetailPageProps> = ({ 
  ...
  const [chapterGroups, setChapterGroups] = useState<Record<string, { label: string; from: number; to: number; chapters: Chapter[] }>>({});
  ...
  useEffect(() => {
    ...
    const groups: Record<string, { label: string; from: number; to: number; chapters: Chapter[] }> = {};
    let lastLabel = '';
  ...
  const userHasAccessToChapter = (chapter: Chapter): boolean => {
    ...
    if (new Date(chapter.releasedAt).getTime() < new Date().getTime() && !chapter.subscribersOnly) {
      return true;
    }
    ...
    for (const subscription of user?.subscriptions || []) {
      ...
      if (manga.subscriptionPlans?.find(
  ...
  const getChapterHistory = (chapterNumber: number) => {
    ...
    return userChapterHistory.find(
  ...
  const getChapterLabel = (chapter: Chapter): string => {
    if (!userHasAccessToChapter(chapter)) {
      return manga.requireLogin ? 'Inicia sesión para leer' : 'Solo para suscriptores';
    }
  ...
  const goToReadChapter = (chapter: Chapter) => {
    if (!logged && manga.requireLogin) {
      window.location.href = `/${organizationSlug || ''}/login?mangaSlug=${manga.slug}&chapterNumber=${chapter.number}&redirect=${window.location.pathname}`.replace('//', '/');
      return;
    }

    if (!userHasAccessToChapter(chapter)) {
      window.location.href = `/${organizationSlug || ''}/subscriptions?mangaSlug=${manga.slug}&chapterNumber=${chapter.number}`.replace('//', '/');
      return;
    }
    ...
    if (logged) {
      ...
      if (history && !history.finishedAt && history.pageNumber) {
        const chapterUrl = organizationSlug 
          ? `/${organizationSlug}/manga/${manga.slug}/chapters/${chapter.number}?page=${history.pageNumber}`
    ...
    const chapterUrl = organizationSlug 
      ? `/${organizationSlug}/manga/${manga.slug}/chapters/${chapter.number}`
  ...
  const toggleFavorite = async (e: React.MouseEvent) => {
    ...
    if (!logged) {
      window.location.href = `/${organizationSlug || ''}/login`.replace('//', '/');
      return;
    }
    ...
    try {
      if (wasFavorite) {
        await callAPI(`/api/favorites/manga-custom/${manga.slug}`, { method: 'DELETE' });
        setFavoriteFeedback({ message: 'Eliminado de favoritos', type: 'success' });
      } else {
        await callAPI(`/api/favorites/manga-custom/${manga.slug}`, { method: 'POST' });
        setFavoriteFeedback({ message: 'Agregado a favoritos', type: 'success' });
      }
      ...
      setIsFavorite(wasFavorite);
      setFavoriteFeedback({ message: 'Error al actualizar favoritos', type: 'error' });
  ...
  const markChapterAsRead = async (e: React.MouseEvent, chapterNumber: number) => {
    e.preventDefault();
    e.stopPropagation();
  ...
  const markChapterAsUnread = async (e: React.MouseEvent, chapterNumber: number) => {
    e.preventDefault();
    e.stopPropagation();
  ...
  // Download chapter
  const downloadChapter = async (e: React.MouseEvent, chapterNumber: number) => {
    e.preventDefault();
    e.stopPropagation();
          ...
          <div className="lg:col-span-3 space-y-8">
            ...
            <div className="space-y-6">
              <div>
                ...
                <div className={`inline-flex px-4 py-2 border rounded-xl text-xs font-black uppercase tracking-widest ${
              ...
              {manga.demography && (
                <div>
                  ...
                  <p className="text-white font-bold text-sm">{manga.demography.name}</p>
                </div>
              )}
            ...
            <div className="space-y-4">
              ...
              <div className="max-w-4xl">
                <p className="text-zinc-300 text-base leading-relaxed font-medium">
                  {manga.description || manga.shortDescription || 'Sin descripción disponible.'}
                </p>
              </div>
            </div>
                ...
                <div className="flex items-center justify-between mb-8">
                  <h2 className="text-3xl font-black text-white italic uppercase tracking-tighter">
                    ...
                    <span className="text-zinc-500 text-sm font-bold ml-2">
                      ...
                      {logged && Array.isArray(userChapterHistory) && userChapterHistory.length > 0 && (
                        <span className="ml-2">
                          ({userChapterHistory.filter((v) => v?.finishedAt).length} leídos)
                        </span>
                      )}
                    </span>
                  </h2>
                  ...
                  <div className="flex-1 space-y-4">
                    {currentChapters
                      ...
                      .map((chapter) => {
                        ...
                        const chapterHistory = getChapterHistory(chapter.number);
                                ...
                                {logged && hasAccess && (
                                  <>
                                    <div className="relative group/tooltip">
                                      {!isRead ? (
                                        <button 
                                          onClick={(e) => markChapterAsRead(e, chapter.number)}
                                          className="hover:text-cyan-400 transition-colors"
                                        >
                                          <Eye size={18} />
                                        </button>
                                        ...
                                        <button 
                                          onClick={(e) => markChapterAsUnread(e, chapter.number)}
                                          className="hover:text-cyan-400 transition-colors"
                                        >
                                          <EyeOff size={18} />
                                        </button>
                                      ...
                                      <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2 py-1 bg-zinc-900 border border-zinc-800 rounded-lg text-white text-[10px] font-bold uppercase tracking-widest whitespace-nowrap opacity-0 group-hover/tooltip:opacity-100 transition-opacity pointer-events-none z-10">
                                        {!isRead ? 'Marcar como leído' : 'Marcar como no leído'}
                                      </div>
                                      {readFeedback && readFeedback.chapterNumber === chapter.number && (
                                    ...
                                    {hasAccess && (
                                      <div className="relative group/tooltip">
                                        <button 
                                          onClick={(e) => downloadChapter(e, chapter.number)}
                                          disabled={isDownloadingChapter === chapter.number}
                                          className="hover:text-cyan-400 transition-colors disabled:opacity-50"
                                        >
                                          {isDownloadingChapter === chapter.number ? (
