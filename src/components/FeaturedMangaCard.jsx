        ...
        <div {...props}>
            <a href={manga?.slug ? `/manga/${manga?.slug}` : '#'}>
                <div className='relative w-full h-full group overflow-hidden rounded-md shadow-md hover:scale-[101%] transition-transform duration-300'>
                    {!manga?.bannerUrl && (
                        <img src={manga?.imageUrl} className='absolute top-1/2 -translate-y-1/2 min-h-full min-w-full object-cover blur-sm duration-300 group-hover:scale-[104%] transition-transform' />
                    )}
                    {!manga?.bannerUrl && (
                        <img src={manga?.imageUrl} className='absolute inset-0 h-full mx-auto object-cover duration-300 group-hover:scale-[102%] transition-transform' />
                    )}
                    {manga?.bannerUrl && (
                        <img src={manga?.bannerUrl} className='absolute top-1/2 -translate-y-1/2 min-h-full min-w-full object-cover blur-sm duration-300 group-hover:scale-[104%] transition-transform' />
                    )}
                    {manga?.bannerUrl && (
                        <img src={manga?.bannerUrl} className='absolute inset-0 w-full min-h-full min-w-full max-h-full mx-auto object-cover duration-300 group-hover:scale-[102%] transition-transform' />
                    )}
