...
import { HelpCircle, Eye, Trophy, Crown, Medal } from 'lucide-react';
...
const ScanTopThree: React.FC<ScanTopThreeProps> = ({ mangas }) => {
    ...
    <section className="relative">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8">
        ...
        <div className="flex items-center gap-2 px-4 py-2 bg-zinc-900/50 border border-zinc-800 rounded-xl text-zinc-500 text-[10px] font-bold uppercase tracking-widest">
          <HelpCircle size={14} /> Visitas totales históricas
        </div>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch">
        {/* RANK #1 - THE MAIN HIGHLIGHT */}
        <div className="lg:col-span-7 group relative h-[500px] md:h-[650px] rounded-[32px] overflow-hidden border border-yellow-500/20 shadow-2xl shadow-yellow-500/5 cursor-pointer">
          <img 
            src={mangas[0].cover} 
          ...
          <div className="absolute top-8 right-8">
            <div className="flex items-center gap-2 bg-black/60 backdrop-blur-md px-3 py-1.5 rounded-full border border-white/10">
              ...
              <span className="text-white font-black text-[10px] tracking-widest">466.181 VISITAS</span>
            </div>
          </div>

          <div className="absolute bottom-10 left-10 right-10">
            <div className="flex flex-wrap gap-2 mb-4">
              {mangas[0].genres?.slice(0, 2).map(g => (
                <span key={g} className="bg-yellow-500/10 text-yellow-500 border border-yellow-500/20 text-[8px] font-black px-2 py-0.5 rounded uppercase tracking-widest">
                  {g}
                </span>
              ))}
            </div>
              ...
              >
                ...
                <div className="absolute bottom-6 left-6 right-6">
                  ...
                  <div className="mt-2 flex items-center gap-2">
                    <span className="text-[10px] font-black text-zinc-500 uppercase tracking-widest">{manga.chapter}</span>
                    <div className="w-1 h-1 rounded-full bg-zinc-700" />
                    <span className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest">{manga.status}</span>
                  </div>
                </div>
              </div>
            );
