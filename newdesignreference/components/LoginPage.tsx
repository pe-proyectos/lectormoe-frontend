...
import { Mail, Lock, User, ArrowRight, ShieldCheck, ArrowLeft, CheckCircle } from 'lucide-react';
        ...
        <div className="bg-zinc-900/60 backdrop-blur-2xl border border-zinc-800 rounded-[40px] p-8 md:p-10 shadow-2xl">
          ...
          <div className="text-center mb-10">
            ...
            {isSubmitted ? (
              ...
              <>
                <h2 className="text-3xl font-black text-white italic uppercase tracking-tighter leading-none mb-2">
                  {view === 'login' && (isScanContext ? 'Ingreso' : 'Bienvenido de nuevo')}
                  {view === 'register' && (isScanContext ? 'Crea tu Perfil' : 'Crea tu cuenta')}
                  {view === 'forgot-password' && 'Recuperar Cuenta'}
                </h2>
                
                <p className="text-zinc-500 text-sm font-medium leading-relaxed">
                  {view === 'login' && (isScanContext ? 'Inicia sesión para sincronizar tu progreso.' : 'Accede a tu historial y apoya a tus scans.')}
                  {view === 'register' && (isScanContext ? 'Regístrate para seguir tus series favoritas.' : 'Únete a la comunidad de manga más premium.')}
                  {view === 'forgot-password' && 'Ingresa tu correo para recibir instrucciones de recuperación.'}
                </p>
              </>
            )}
          </div>
